...
import pyttsx3, os, requests, pymongo
import wavio as wv
import json
from voice_to_voice import disorder
...
def load(fname):
    import json
    name = 'E:\projects\psychological-ai-backend\\api\\voice_to_voice\data\\' + fname + '.json'
    with open(name) as f:
        res = json.load(f)
    return res
        ...
        while (1):
            try:
                with sr.Microphone() as source2:
                    ...
                    with open('E:\projects\psychological-ai-backend\\api\\voice_to_voice\\recording\classify\\' + name + '.wav', 'wb') as f:
            ...
            while (1):
                try:
                    with sr.Microphone() as source2:
                        ...
                        audio2 = r.listen(source2)
                        name = '_'.join(k.split())
                        with open('E:\projects\psychological-ai-backend\\api\\voice_to_voice\\recording\\' + test + '\\' + name + '.wav', 'wb') as f:
                            f.write(audio2.get_wav_data())
